...
# 🦁 XXD Panel 075｜어두운 크레용 스케치

### 짙은 크레용, 부드러운 색면, 종이의 여백으로 사진의 가장 중요한 한 획을 남깁니다
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 075이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

흔한 ‘손그림 빈티지’ 처리는 사진을 지나치게 묘사하거나 낡은 종이 필터 하나로 덮어 버립니다. 피사체는 남지만 멈춤, 거리, 사적인 기억은 사라집니다.

**Panel 075**는 피사체의 핵심 비율, 방향, 구조, 자세를 남기고 멈춤과 가벼운 수정 흔적을 품은 짙은 크레용 선으로 다시 그립니다. 저채도 불규칙 색면 하나는 피사체를 받칠 뿐 화면을 채우지 않으며, 아이보리 수제 종이와 넓은 여백이 감정, 거리, 시간을 담습니다. 작은 타자기 글자는 상업 제목이 아니라 사적인 메모처럼 놓입니다.

### 이런 경우에 적합합니다

- 템플릿 빈티지 필터 대신 선, 종이, 입자, 생략 자체에 의미를 두고 싶을 때
- 적게 그리면서도 피사체의 비율, 방향, 구조, 자세를 정확히 유지하고 싶을 때
- 부드러운 단일 색면, 리소그래프 입자, 넓은 여백, 절제된 타자기 메모를 좋아할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 075로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 짙은 크레용 속사 · 아이보리 수제 종이 · 부드러운 불규칙 색면 · 리소그래프 입자 · 넓은 여백 · 타자기 사적 메모
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 사적인 독립 출판물의 한 페이지 같은 절제된 종이 스케치 포스터 |
| **대표 특징** | 아이보리 수제 종이와 하나의 부드러운 색면 위 짙은 크레용 선, 은은한 리소그래프 입자 |
| **입력 존중 방식** | 세부를 생략해도 핵심 비율, 방향, 구조, 자세, 관계를 유지하며 색은 원본 지시문을 따름 |
...
```text
피사체·윤곽·자세·관계 추출 → 식별에 필요한 구조만 남기기 → 느슨한 짙은 크레용으로 다시 그리기 → 저채도 불규칙 색면 하나로 받치기 → 아이보리 수제 종이에 넓은 여백 남기기 → 은은한 리소그래프 입자 더하기 → 최소한의 타자기 사적 메모 놓기
```

## 완성작의 특징

- 짙은 크레용 선은 멈춤, 반복, 수정을 허용하지만 핵심 구조는 정확하게 유지합니다.
- 하나의 부드러운 저채도 불규칙 색면이 피사체를 받치며 배경 전체를 채우지 않습니다.
- 아이보리 수제 종이, 은은한 리소그래프 입자, 넓은 여백이 촉감, 거리, 시간을 만듭니다.
- 원본에서 가장 인상적인 구조 관계에 따라 중간이나 아래에 배치하며 기계적으로 중앙 정렬하지 않습니다.
- 작고 가벼운 타자기 글자는 기계적 오차를 품은 사적 메모이며 상업 제목이 아닙니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-075-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 075의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-075 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-075 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-075 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-075 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-075 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-075 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 075 적용, 공통 설정은 한 번만 확정
/xxd-panel-075 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-075`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-075-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-075-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

75개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 075까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–075) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
