<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 075 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 075 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
提取主体、轮廓、姿态与关系 → 省略到最关键的识别结构 → 以松散深色蜡笔线重新落笔 → 用一块低饱和不规则色域承托 → 在米白手工纸上保留大面积空白 → 加入淡淡 Risograph 颗粒 → 以极少打字机小字留下私人注记
```

## 成品中最容易识别的风格特点

- 深色蜡笔线松散、随性，允许停顿、轻微重复与修正，但关键结构保持准确。
- 一块柔和低饱和不规则色域呼应原图综合色调，只承托主体，不填满背景。
- 米白手工纸、淡淡 Risograph 颗粒和大量留白共同建立触感、距离与时间。
- 主体可落在中部或偏下，由原图最动人的结构关系决定，不机械居中。
- 文字是小型、低权重、略带机械误差的打字机私人注记，不是商业标题。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-075-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–075） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
