...
# 🦁 XXD Panel 075｜ダーククレヨン画稿

### 濃色クレヨン、柔らかな色面、紙の呼吸で写真の大切な一筆を残す
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 075 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

一般的な「手描きレトロ」は写真を描き込みすぎるか、均一な古紙フィルターを重ねがちです。主体は残っても、間、距離、私的な記憶が失われます。

**Panel 075** は主体の重要な比率、方向、構造、姿勢を残し、止まりや軽い描き直しを含む濃色クレヨン線で描きます。低彩度の不定形色面は主体を支えるだけで埋め尽くさず、アイボリーの手工紙と大きな余白が感情、距離、時間を担います。小さなタイプライター文字は商業見出しでなく私的な注記です。

### 向いている用途

- テンプレート的なレトロフィルターではなく、線、紙、粒子、省略そのものに意味を持たせたい場合
- 描く量を減らしても、主体の重要な比率、方向、構造、姿勢を正確に保ちたい場合
- 柔らかな単一色面、リソグラフ粒子、大きな余白、控えめなタイプライター注記を好む場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 075 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 濃色クレヨン速写 · アイボリー手工紙 · 柔らかな不定形色面 · リソグラフ粒子 · 大きな余白 · タイプライター私的注記
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 私的なインディペンデント出版物の一頁のような、抑制された紙面速写ポスター |
| **識別しやすい特徴** | アイボリー手工紙と一つの柔らかな色面に、濃色クレヨン線と淡いリソグラフ粒子 |
| **入力の尊重** | 細部を省略しても重要な比率、方向、構造、姿勢、関係を保ち、色は原文指示に従う |
...
```text
主体、輪郭、姿勢、関係を抽出 → 識別に必要な構造まで省略 → 濃色クレヨンでゆるく描き直す → 低彩度の不定形色面一つで支える → アイボリー手工紙の大きな余白を残す → 淡いリソグラフ粒子を加える → 最小限のタイプライター私的注記を置く
```

## 完成作の特徴

- 濃色クレヨン線は止まり、反復、修正を許しながら、重要な構造は正確に保つ。
- 一つの柔らかな低彩度不定形色面が主体を支え、背景全体を埋めない。
- アイボリー手工紙、淡いリソグラフ粒子、大きな余白が触感、距離、時間をつくる。
- 元写真で最も動きのある構造関係に応じ、中部または下部へ着地させ、機械的に中央配置しない。
- 小さく軽い、機械誤差のあるタイプライター文字は商業見出しでなく私的注記になる。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-075-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 075 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-075 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-075 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-075 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-075 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 075 を適用し、共通設定は一度だけ解決
/xxd-panel-075 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-075`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-075-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-075-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全プロジェクト一覧

75 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 075 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–075） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
